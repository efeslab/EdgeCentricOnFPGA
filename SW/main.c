...
graph_t *graph_init(int num_v, int num_e, char *filename)
{
    ...
    int num_intervals = (num_v - 1) / VERTEX_PER_INTERVAL + 1;
    g->intervals = malloc(sizeof(interval_t)*num_intervals);
    ...
    for (i = 0; i < num_intervals; i++) {
        ...
        g->intervals[i].update_bin = malloc(sizeof(update_t) * NUM_UPDATE_BIN_ENTRIES);
